...
def data_loading_fsk_v1(chunksize: Optional[int] = None) -> Optional[Union[pd.DataFrame, Iterator[pd.DataFrame]]]:
    ...
    WHERE f.feature_slug IS NOT NULL 
        ...
        AND f.feature_slug = 'fsk_v2.0' 
        AND l.loan_status != 'never'
    """
    ...
    try:
        ...
        def process_chunk(chunk):
            ...
            processed_chunk = extract_db_data(chunk, 5)
            
            # Verify columns exist
            missing_cols = [col for col in numerical_columns + categorical_columns if col not in processed_chunk.columns]
            if missing_cols:
            ...
            for col in numerical_columns:
                processed_chunk[col] = processed_chunk[col].replace([np.inf, -np.inf], np.nan).fillna(0).astype(np.int64)
        ...
        if chunksize:
            ...
            if not processed_chunks:
                logger.warning("No valid chunks after processing; returning empty DataFrame")
                return pd.DataFrame(columns=numerical_columns + categorical_columns + ['user_id', 'cid', 'user_status', 'loan_status', 'payoff_score', 'feature_slug', 'answer'])
            return pd.concat(processed_chunks, ignore_index=True)
        else:
            logger.info("Loading full dataset")
            plain_dat = pd.read_sql(query, conn)
            raw_dat = extract_db_data(plain_dat, 5)

            # Verify columns exist
            missing_cols = [col for col in numerical_columns + categorical_columns if col not in raw_dat.columns]
            if missing_cols:
...
def extract_db_data(data: pd.DataFrame, min_payoff_score: float) -> pd.DataFrame:
    attr_mapping = [
    ...
    for index, dat in data.iterrows():
        try:
            ...
            if (dat['user_status'] == 'normal' and 
                dat['loan_status'] == 'healthy' and 
                dat['payoff_score'] < min_payoff_score):
                continue
                
            # Assign 'ust' based on user_status and loan_status
            data.at[index, 'ust'] = 0 if (dat['user_status'] == 'normal' and dat['loan_status'] == 'healthy') else 1
    ...
    if not filtered_indices:
        ...
        return pd.DataFrame(columns=[attr for attr, _, _ in attr_mapping] + ['ust'] + ['user_id', 'cid', 'user_status', 'loan_status', 'payoff_score', 'feature_slug', 'answer'])
    return data.loc[filtered_indices].copy()
        ...
        if isinstance(raw_dat, pd.DataFrame):
            ...
            for i, chunk in enumerate(raw_dat):
                if chunk is not None:
                    ...
                    logger.info(f"Chunk {i+1} Columns: {chunk.columns.tolist()}")
                    logger.info(f"Chunk {i+1} Dtypes: {chunk.dtypes.to_dict()}")
                    logger.info(f"Chunk {i+1} Sample (5 rows):\n{chunk.sample(5).to_string()}")
                else:
