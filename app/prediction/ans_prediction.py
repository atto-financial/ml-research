...
from app.utils.utils_model import validate_data
from app.utils.utils_model import get_artifact_paths, load_and_verify_artifact, load
from app.prediction.utils_prediction import screen_fsk_answers, predict_answers
from app.utils.feature_engine import FeatureEngine
...
# Initialize FeatureEngine with shared config
CONFIG_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), 'config', 'feature_config.yaml')
...
def fsk_answers_v2(answers: Dict, model_path: str = None, scaler_path: str = None, metadata_path: str = None, drop_sum_columns: bool = True) -> Tuple[Dict, int]:
    try:
        ...
        # Replicate dbt logic using shared config
        cus_engineered_data = feature_engine.engineer_features(cus_ans_data)
        
        if not validate_data(cus_engineered_data, "Engineered data"):
            logger.error("Feature engineering failed in FeatureEngine")
        ...
        paths = get_artifact_paths(model_dir="artifacts/models", model_path=model_path, scaler_path=scaler_path)
...
FEAST_REPO_PATH = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(__file__))), 'feature_repo')
try:
    feature_store = FeatureStore(repo_path=FEAST_REPO_PATH)
except Exception as e:
    logger.warning(f"Failed to initialize Feast FeatureStore: {e}")
    feature_store = None

def fk_answers_v1(answers: Dict, metadata_path: str = None, model_path: str = None, scaler_path: str = None, drop_sum_columns: bool = True, user_id: Optional[str] = None) -> Tuple[Dict, int]:
    try:
        # If user_id is provided, try fetching from Feast
        if user_id and feature_store:
            logger.info(f"Fetching features from Feast for user_id: {user_id}")
            # Pull full features from Feast (which were generated by dbt)
            feature_vector = feature_store.get_online_features(
                features=[
                    "loan_features:fht1", "loan_features:fht2", "loan_features:fht3", "loan_features:fht4",
                    "loan_features:fht5", "loan_features:fht6", "loan_features:fht7", "loan_features:fht8",
                    "loan_features:kmsi1", "loan_features:kmsi2", "loan_features:kmsi3", "loan_features:kmsi4",
                    "loan_features:kmsi5", "loan_features:kmsi6", "loan_features:kmsi7", "loan_features:kmsi8",
                    # Add engineered features if needed, but we can also re-calculate them here for safety
                ],
                entity_rows=[{"user_id": user_id}]
            ).to_dict()
            
            # Combine or override provided answers with Feast features
            logger.debug(f"Feast features: {feature_vector}")
            
            if feature_vector.get('loan_features__fht1') and not answers.get('fht'):
                answers['fht'] = [feature_vector[f'loan_features__fht{i+1}'][0] for i in range(8)]
            if feature_vector.get('loan_features__kmsi1') and not answers.get('kmsi'):
                answers['kmsi'] = [feature_vector[f'loan_features__kmsi{i}'][0] for i in range(1, 9)]
        ...
        # Engineering logic using shared config
        cus_engineered_data = feature_engine.engineer_features(cus_ans_data)
