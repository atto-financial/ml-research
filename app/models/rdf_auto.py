...
def backward_elimination(X: pd.DataFrame, y: pd.Series, target_col: str = 'ust', scoring: str = 'roc_auc') -> Tuple[list, RandomForestClassifier, dict]:
    """
    Perform backward elimination to select the best subset of features using ROC AUC.
    Returns the best selected features, the best model, and the best metrics.
    """
    try:
...
def train_random_forest_model(clean_engineer_dat: pd.DataFrame, selected_features: list, target_col: str = 'ust') -> Tuple[Optional[RandomForestClassifier], Optional[dict]]:
    try:
                    ...
                    if result is not None:
                        ...
                        if corr_dat is not None:
                            ...
                            if selected_features:
                                model, metrics = train_random_forest_model(clean_engineer_dat, selected_features)
